...
token.string <- readLines("key.txt", n=1)
...
write.csv(Mprim, file = "~/QueryResults_SDG_1.csv")
 ...
 for (x in 2:16) #For loop, to get every SDG separately queried
 {
   ...
   query.string <- paste("search publications where (year in [2018:2022]) and (type in [\"article\"]) and (research_orgs.id = \"grid.25073.33\") and (category_sdg.id = ",toString(curr_sdg),") return publications[basics + extras + categories + concepts]", sep = '')
   ...
   files.name <- paste("~/QueryResults_SDG_",toString(x),".csv",sep = '')
   write.csv(M, file = files.name)
 }
